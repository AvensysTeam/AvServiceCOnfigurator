...
const CAPComponent = ({navigation}: any) => {
  ...
  const handleConfirm = () => {
    ...
    let CAP: ICAP = { value1: value1, value2: value2, value3: value3, confirm1: true, confirm2: true, confirm3: true, confirm: true}
    temp.CAP = CAP
    tempList[roomNumber] = temp
    setRoomList(tempList)
    navigation.navigate("CO2", {from: "CAP"})
  }
  ...
  useEffect(() => {
    const temp = roomList[roomNumber].CAP
    if (temp != undefined) {
  ...
  return (
    <ScrollView style={styles.container}>
      <View style={{ alignItems: 'center', position: 'relative', width: '100%', height: Dimensions.get("window").height * 0.9 }}>
        <_Text text={`${roomList[roomNumber].unitname} (CAP)`} size={20} lineHeight={24} />
        <View style={{ position: 'relative' }}>
          <View style={styles.body}>
            <Image source={ImageSource.ico_ktp} style={{ width: 200, height: 200, resizeMode: 'cover' }} />
          </View>
          <View style={styles.item_group}>
            <TouchableOpacity style={styles.item} onPress={() => setStart3(true)}></TouchableOpacity>
            <TouchableOpacity style={styles.item} onPress={() => setStart2(true)}></TouchableOpacity>
            <TouchableOpacity style={styles.item} onPress={() => setStart1(true)}></TouchableOpacity>
          </View>
        ...
        <View style={{ width: '70%', gap: 10, marginTop: 10, alignItems: 'center' }}>
          {start3 && <View style={styles.form}>
            <_Text text="Pressure 3: " size={18} lineHeight={20} />
            <_Input value={value3} setValue={setValue3} width={100} />
            <_Text text="Pa" size={18} lineHeight={20} />
          </View>}
          {start2 && <View style={styles.form}>
            <_Text text="Pressure 2: " size={18} lineHeight={20} />
            <_Input value={value2} setValue={setValue2} width={100} />
            <_Text text="Pa" size={18} lineHeight={20} />
          </View>}
          {start1 && <View style={styles.form}>
            <_Text text="Pressure 1: " size={18} lineHeight={20} />
