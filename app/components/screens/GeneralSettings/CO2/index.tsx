...
const CO2Component = ({ navigation }: any) => {
  ...
  const handleConfirm = () => {
    ...
    let CO2: ICO2 = { threshold: threshold, boost: boost }
    temp.CO2 = CO2
    tempList[roomNumber] = temp
    setRoomList(tempList)
    navigation.navigate("RH", {from: 'CO2'})
  }

  useEffect(() => {
    const temp = roomList[roomNumber].CO2
    if (temp != undefined) {
  ...
  return (
    <View style={styles.container}>
      <_Text text={`${roomList[roomNumber].unitname} (CO2)`} size={24} lineHeight={24} />
      <View style={{ gap: 20 }}>
