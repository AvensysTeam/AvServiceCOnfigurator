...
import { IRH, IRoom } from "../../../../types/interface";

const RHComponent = ({ navigation }: any) => {
  ...
  const handleConfirm = () => {
    ...
    let RH: IRH = { threshold: threshold, boost: boost }
    temp.RH = RH
    tempList[roomNumber] = temp
    setRoomList(tempList)
    navigation.navigate("VOC", {from: 'RH'})
  }

  useEffect(() => {
    const temp = roomList[roomNumber].RH
    if (temp != undefined) {
  ...
  return (
    <View style={styles.container}>
      <_Text text={`${roomList[roomNumber].unitname} (RH)`} size={24} lineHeight={24} />
      <View style={{ gap: 20 }}>
