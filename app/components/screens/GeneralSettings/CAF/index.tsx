...
const CAFComponent = ({ navigation }: any) => {
  ...
  const handleConfirm = () => {
    ...
    let CAF: ICAF = { value1: value1, value2: value2, value3: value3, confirm1: true, confirm2: true, confirm3: true, confirm: true}
    temp.CAF = CAF
    tempList[roomNumber] = temp
    setRoomList(tempList)
    navigation.navigate("CAP", {from: "CAF"})
  }
  ...
  useEffect(() => {
    const temp = roomList[roomNumber].CAF
    if (temp != undefined) {
  ...
  return (
    <ScrollView style={styles.container}>
      <View style={{ alignItems: 'center', position: 'relative', width: '100%', height: Dimensions.get("window").height * 0.9 }}>
        <_Text text={`${roomList[roomNumber].unitname} (CAF)`} size={20} lineHeight={24} />
        ...
        <View style={{ width: '70%', gap: 10, marginTop: 20, alignItems: 'center' }}>
          {start3 && <View style={styles.form}>
            <_Text text="Speed 3: " size={18} lineHeight={20} />
            <_Input value={value3} setValue={setValue3} width={100} />
            <_Text text="m/h" size={18} lineHeight={20} />
          </View>}
          {start2 && <View style={styles.form}>
            <_Text text="Speed 2: " size={18} lineHeight={20} />
            <_Input value={value2} setValue={setValue2} width={100} />
            <_Text text="m/h" size={18} lineHeight={20} />
          </View>}
          {start1 && <View style={styles.form}>
            <_Text text="Speed 1: " size={18} lineHeight={20} />
